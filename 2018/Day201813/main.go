...
func getData(input string) ([]string, []cart) {
	...
	for y, line := range strings.Split(input, "\n") {
		...
		for x, char := range line {
			if char == 60 {
				carts = append(carts, cart{270, x, y, 0})
			}
			if char == 62 {
				carts = append(carts, cart{90, x, y, 0})
			}
			if char == 118 {
				carts = append(carts, cart{180, x, y, 0})
			}
			if char == 94 {
				carts = append(carts, cart{0, x, y, 0})
			}
		}
	}
...
func moveCart(c cart, grid []string, carts []cart) (cart, bool) {
	...
	if c.x < 0 || c.y < 0 || c.y >= len(grid) || c.x >= len(grid[c.y]) || (c.y == 30 && c.direction == 270) {
		fmt.Println("OOPS")
	}
